...
create table `user` (
  ...
  role enum("ADMIN", "ASSISTANT", "CLIENT") not null,
...
'Лихачёв', 'Абрам', 'Миронович', 'MALE', 182766, 9899,
'ADMIN', '89001111111', 'pass');

insert into `user`(login, surname, name, patronymic, sex, passport_series,
passport_number, role, phone, password) values ('assistant1',
'Трофимова', 'Зинаида', 'Тихоновна', 'FEMALE', 900999, 2987,
'ASSISTANT', '89002222222', 'pass');

insert into `user`(login, surname, name, patronymic, sex, passport_series,
passport_number, role, phone, password) values ('assistant2',
'Филатов', 'Денис', 'Дмитрьевич', 'MALE', 987384, 3444,
'ASSISTANT', '89002378769', 'pass');

insert into `user`(login, surname, name, patronymic, sex, passport_series,
passport_number, role, phone, password) values ('client1',
'Мишина', 'Наталья', 'Евсеевна', 'FEMALE', 946512, 3965,
'CLIENT', '89003333333', 'pass');

insert into `user`(login, surname, name, patronymic, sex, passport_series,
passport_number, role, phone, password) values ('client2',
'Артемьев', 'Варлаам', 'Тимофеевич', 'MALE', 236779, 7462,
'CLIENT', '89004444444', 'pass');
...
create table request (
    ...
    status enum("CREATED", "APPLIED", "IN_WORK", "CANCELED", "FINISHED", "CLIENT_IS_AWARE") not null,
...
('Петров', 'Петр', 'Петрович', 'MALE', 123456, 1234, '2020-06-25 15:00:00',
'CREATED',
(select id from `user` where login = 'client1'));

insert into request (surname, name, patronymic, sex, passport_series,
    passport_number, arrival_time, status, client_id) values
('Петрова', 'Алена', 'Ивановна', 'FEMALE', 777777, 7777, '2020-06-26 18:00:00',
'CREATED',
(select id from `user` where login = 'client1'));

insert into request (surname, name, patronymic, sex, passport_series,
    passport_number, arrival_time, status, client_id) values
('Кириллова', 'Анна', 'Андреевна', 'FEMALE', 248732, 2873, '2020-06-26 15:30:00',
'CREATED',
...
create table form (
    ...
    status enum("IN_WORK", "FINISHED") not null,
    foreign key (request_id) references request(id),
    foreign key (analysis_id) references analysis(id),
...
create table busy (
    ...
    end_time time not null,
    form_id int not null,
    reason enum('COLLECTION', 'RESEARCH') not null,
    foreign key (assistant_id) references `user`(id),
    foreign key (form_id) references form(id),
